...
import { setCorrect, setWrong, setScore, answersRecord, saveNewBadges } from "../../../actions";

const { Panel } = Collapse;

const QuizStep = ({ singleMCQ, nextStep, setIndex, countdownRef, studentBio }) => {
	...
	const onSubmitClick = (e) => {
		...
		if (selectedValue !== "") {
			...
			selectedValue === singleMCQ.Answer
				? dispatch(setCorrect(1)) && dispatch(setScore(10)) && dispatch(answersRecord(true))
	...
	const onNextClick = () => {
		...
		if (singleMCQ.count === 9) {
			...
			if (
				...
				axios
					.post(
						`http://localhost:4000/api/badges/newbadges/${studentBio.StdID}/${courseid}/${stid}/${difficulty}/${stats.score}/${learnerType}`
					)
					.then((response) => {
				...
				switch (difficulty) {
					case 1:
						axios
							.put(`http://localhost:4000/api/scores/${studentBio.StdID}/${stid}`, null, {
	...
	return (
		...
		>
			<Stats />
			<br />
			<Card title={<h5>{"#" + (singleMCQ.count + 1) + " " + singleMCQ.Question}</h5>} bordered={false}>
				{
					<Radio.Group onChange={(e) => onRadioChange(e)} value={selectedValue} id="radio">
				...
				<button
					...
					id="nextBtn"
					style={{ backgroundColor: "#66BB6A", color: "white" }}
					onClick={() => (submitted ? onNextClick() : message.error("Submit your answer first!"))}
				>
					Next
					<CaretRightOutlined />
				</button>
