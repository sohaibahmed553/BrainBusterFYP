...
const Profile = (props) => {
	...
	const loadStudentData = () => {
		try {
			...
			const student = jwtDecode(jwt);

			axios.get("http://localhost:4000/api/students/" + student.student.StdID).then((res) => {
				setData(res.data);
			});
		} catch (err) {}
	};
	...
	const onUpdate = (values, id) => {
		...
		axios
			.put("http://localhost:4000/api/auth/" + id, {
				Gender,
				Email,
				NickName,
				Pass,
			...
			{data.length && (
				...
				>
					...
					>
						<Input
							prefix={
								<UserOutlined className="site-form-item-icon" style={{ color: "rgba(0,0,0,.25)" }} />
							}
						/>
					</Form.Item>
					{/*--------------------Email-------------- */}
					...
					>
						<Input.Password
							prefix={
								<LockOutlined className="site-form-item-icon" style={{ color: "rgba(0,0,0,.25)" }} />
							}
							type="password"
						/>
					</Form.Item>
