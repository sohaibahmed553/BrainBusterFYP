...
const Comments = (props) => {
	...
	const loadReplies = React.useCallback(async () => {
		axios.get("http://localhost:4000/api/comments/replies/" + props.comments.CommentID).then((res) => {
			setReplies(res.data);
		});
	}, [props.comments]);

	const loadReplyCount = React.useCallback(async () => {
		axios.get("http://localhost:4000/api/comments/replycount/" + props.comments.CommentID).then((res) => {
			...
			<div className="comment-box shadow p-3 mb-3 bg-white rounded ">
				<div className="row">
					...
					<div className="col-lg-2">
						{props.comments.StdID === props.studentBio.StdID && (
							<React.Fragment>
								<DeleteComment comment={props.comments} onChangeComment={props.onChangeComment} />
								<EditComment comment={props.comments} onChangeComment={props.onChangeComment} />
							</React.Fragment>
						)}
					</div>
				</div>
