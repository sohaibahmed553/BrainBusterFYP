...
const helperFucntion1 = function* (action) {
	...
	if (courseid === "") return;
	yield axios.get(`http://localhost:4000/api/students/dashboard/${action.payload}/${courseid}`).then((response) => {
...
const helperFunction2 = function* (action) {
	...
	const courseid = params.get("courseid");
	if (courseid === "") return;
	yield axios.get(`http://localhost:4000/api/stages/stagedetails/${action.payload}/${courseid}`).then((response) => {
...
const helperFunction3 = function* () {
	...
	yield axios.get(`http://localhost:4000/api/questions/${stid}/${difficulty}`).then((response) => {
...
const helperFunction4 = function* () {
	...
	const params = new URLSearchParams(window.location.search);
	const courseid = params.get("courseid");
	yield axios.get(`http://localhost:4000/api/scores/leaderboard/${courseid}`).then((response) => {
...
const helperFucntion5 = function* () {
	let coursesPageData;
	yield axios.get("http://localhost:4000/api/courses/").then((response) => {
...
const helperFucntion6 = function* () {
	...
	const courseid = params.get("courseid");
	if (courseid === "") return;
	yield axios.get(`http://localhost:4000/api/courses/data/${courseid}`).then((response) => {
