...
//@desc Get all Questions by passing Stage and dificulty for instructor
//@access Public
router.get("/show/:stid/:difficulty", async (req, res) => {
  let sql = "SELECT * FROM Questions where StID = ? and difficulty = ?";
...
//@route GET api/questions/check/:stid/:difficulty
//@desc Get count of questions by passing the stageid and difficulty
//@access Public
router.get("/count/:stid/:difficulty", async (req, res) => {
  let sql =
    "SELECT count(questions) as noofquestions from Questions where StId = ? and difficulty = ?";
  let query = await conn.query(
    sql,
    [req.params.stid, req.params.difficulty],
    (err, results) => {
...
router.get("/:stid", async (req, res, next) => {
  ...
  var hardQuestions = [];
  let sql1 =
    "SELECT * FROM Questions where StID=? and Difficulty=1 order BY RAND() limit 10";
  let sql2 =
    "SELECT * FROM Questions where StID=? and Difficulty=2 order BY RAND() limit 10";
  let sql3 =
    "SELECT * FROM Questions where StID=? and Difficulty=3 order BY RAND() limit 10";
...
//@route GET api/questions/:StID/:difficulty
//@desc Get Questions by passing Stage and dificulty #quizpage
//@access Public
router.get("/:stid/:difficulty", async (req, res) => {
  sql1 =
    "select StTitle, CourseName from Stages, Courses where Stages.courseid= Courses.courseid and StID=?";
  sql2 =
    "SELECT Questions,A,B,C,D,Answer,Explanation FROM `Questions` WHERE `StID` = ? and `Difficulty` = ? order BY RAND() limit 10";
  let finalObject = {};
  conn.query(
    sql1 + ";" + sql2,
    [req.params.stid, req.params.stid, req.params.difficulty],
    (err, results) => {
      finalObject = results[0][0];
      finalObject.questions = results[1];
...
router.post(
  ...
  [
    ...
    check("difficulty", "Please enter difficulty of question.")
      .not()
      .isEmpty()
      .isLength({ max: 1 }),
