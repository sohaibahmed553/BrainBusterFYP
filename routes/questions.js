...
router.get("/show/:stid/:difficulty", async (req, res) => {
	...
	let query = await conn.query(sql, [req.params.stid, req.params.difficulty], (err, results) => {
		res.send(results);
	});
});
...
router.get("/count/:stid/:difficulty", async (req, res) => {
	let sql = "SELECT count(questions) as noofquestions from questions where StId = ? and difficulty = ?";
	let query = await conn.query(sql, [req.params.stid, req.params.difficulty], (err, results) => {
		res.send(results[0]);
	});
});
...
router.get("/:stid/:difficulty", async (req, res) => {
	sql1 = "select StTitle, CourseName from stages, courses where stages.courseid= courses.courseid and StID=?";
	sql2 =
		"SELECT Questions,A,B,C,D,Answer,Explanation FROM `questions` WHERE `StID` = ? and `Difficulty` = ? order BY RAND() limit 10";
	let finalObject = {};
	conn.query(sql1 + ";" + sql2, [req.params.stid, req.params.stid, req.params.difficulty], (err, results) => {
...
router.put(
    ...
    [
        ...
        check("answer", "Please select answer.")
            ...
            .isEmpty()
            .isLength({ max: 1 }),
        check("explanation", "Please enter explanation of answer.")
            .not()
            .isEmpty(),
    ],
    async (req, res) => {
