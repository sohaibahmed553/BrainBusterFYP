...
router.post("/newbadges/:stdid/:courseid/:stid/:difficulty/:score/:learnertype", async (req, res, next) => {
	...
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 1 && x.CourseID === courseid).length === 0) {
			conn.query(
				`
				...
				`,
				(err, results) => {
					let countQuiz = results[0][0].easyCount + results[0][0].easyCount + results[0][0].easyCount;
					if (countQuiz === 0) {
						conn.query(`insert into studentbadges values ( ${stdid}, 1, ${courseid} )`);
						earnedBadges.push({
							BName: "Guru",
							BAvatar: "Guru.png",
						});
	...
	// 2.Intelligent
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 2 && x.CourseID === courseid).length === 0) {
			conn.query("", () => {
				if (score === 100 && difficulty === 3) {
					conn.query(`insert into studentbadges values ( ${stdid}, 2, ${courseid} )`);
					earnedBadges.push({
						BName: "Intelligent",
	...
	// 3.Ten to the third
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 3 && x.CourseID === courseid).length === 0) {
			conn.query(
				`select TotalScore from scoresofstudents where courseid=${courseid} and stdid=${stdid}`,
				(err, results) => {
					if (results[0].TotalScore >= 1000) {
						conn.query(`insert into studentbadges values ( ${stdid}, 3, ${courseid} )`);
						earnedBadges.push({
	...
	// 4.Quiz Master
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 4 && x.CourseID === courseid).length === 0) {
			conn.query(
				`
				...
				`,
				(err, results) => {
					let countPassedQuizes = results[0][0].easyCount + results[0][0].easyCount + results[0][0].easyCount;
					if (countPassedQuizes >= 10) {
						conn.query(`insert into studentbadges values ( ${stdid}, 4, ${courseid} )`);
						earnedBadges.push({
							BName: "Quiz Master",
	...
	// 5.Banjamin Franklin
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 5 && x.CourseID === courseid).length === 0) {
			conn.query(
				`select StdID from scoresofstudents where courseid=${courseid} order by totalscore desc limit 1`,
				(err, results) => {
					if (results[0].StdID === stdid) {
						conn.query(`insert into studentbadges values ( ${stdid}, 5, ${courseid} )`);
						earnedBadges.push({
	...
	// 6.Quick Learner
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 6 && x.CourseID === courseid).length === 0) {
			conn.query("", () => {
				if (learnerType === "Quick" && difficulty === 1) {
					conn.query(`insert into studentbadges values ( ${stdid}, 6, ${courseid} )`);
					earnedBadges.push({
						BName: "Quick Learner",
	...
	// 7.Rapid Learner
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 7 && x.CourseID === courseid).length === 0) {
			conn.query("", () => {
				if (learnerType === "Rapid" && difficulty === 2) {
					conn.query(`insert into studentbadges values ( ${stdid}, 7, ${courseid} )`);
					earnedBadges.push({
						BName: "Rapid Learner",
	...
	// 8.Clever Learner
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 8 && x.CourseID === courseid).length === 0) {
			conn.query("", () => {
				if (learnerType === "Clever" && difficulty === 3) {
					conn.query(`insert into studentbadges values ( ${stdid}, 8, ${courseid} )`);
	...
	// 9.Simple as ABC
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 9 && x.CourseID === courseid).length === 0) {
			conn.query(
				`select count(easyscore) as easyCount from stages, score where stages.stid = score.stid and stdid=${stdid} and courseid=${courseid} and easyscore=0;`,
				(err, results) => {
					console.log("easy ", results[0].easyCount);
					if (results[0].easyCount === 0) {
						conn.query(`insert into studentbadges values ( ${stdid}, 9, ${courseid} )`);
						earnedBadges.push({
							BName: "Simple As ABC",
	...
	// 10.Intermediate
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 10 && x.CourseID === courseid).length === 0) {
			conn.query(
				`select count(mediumscore) as mediumCount from stages, score where stages.stid = score.stid and stdid=${stdid} and courseid=${courseid} and mediumscore=0;`,
				(err, results) => {
					console.log("medium ", results[0].mediumCount);
					if (results[0].mediumCount === 0) {
						conn.query(`insert into studentbadges values ( ${stdid}, 10, ${courseid} )`);
						earnedBadges.push({
							BName: "Intermediate",
	...
	// 11.Victory
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 11 && x.CourseID === courseid).length === 0) {
			conn.query(
				`select count(hardscore) as hardCount from stages, score where stages.stid = score.stid and stdid=${stdid} and courseid=${courseid} and hardscore=0;`,
				(err, results) => {
					console.log("hard ", results[0].hardCount);
					if (results[0].hardCount === 0) {
						conn.query(`insert into studentbadges values ( ${stdid}, 11, ${courseid} )`);
						earnedBadges.push({
							BName: "Victory",
							BAvatar: "Victory.png",
						});
	...
	// 12.First Success
	conn.query("select * from studentBadges", (err, results) => {
		if (results.filter((x) => x.StdID === stdid && x.BID === 12 && x.CourseID === courseid).length === 0) {
			conn.query("", () => {
				conn.query(`insert into studentbadges values ( ${stdid}, 12, ${courseid} )`);
				earnedBadges.push({
					BName: "First Success",
...
router.delete("/:id", async (req, res) => {
	conn.query("Delete FROM `badges` WHERE BID = ?", req.params.id, (err, results) => {
		res.send(results);
	});
});
