...
import { Row, Col, Select, Form, Empty } from "antd";
...
const ManageStages = (props) => {
  ...
  const loadStages = async (courseid) => {
    //setLoading(true);
    await axios.get("/api/stages/" + courseid).then((res) => {
      setStages(res.data);
      setLoading(false);
    });
  };
  ...
  const onCourseChange = (courseid) => {
    setCourseOnEdit(courseid);
    loadStages(courseid);
  };

  const loadData = () => {
    axios.get("/api/stages/" + courseOnEdit).then((res) => {
      setStages(res.data);
    });
  };
    ...
    <div>
      ...
      {stages.length === 0 ? (
        ...
        <table className="table table-striped table-sm table-bordered small">
          <thead>
            <tr>
              <th className="text-center">Stage Title</th>
