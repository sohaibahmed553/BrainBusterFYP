...
const SingleCourse = (props) => {
  const checkAllowed = async () => {
    let allow;
    await axios.get("/api/courses/allow/" + props.data.CourseID).then((res) => {
  ...
  const changeCourseStatus = async () => {
    if (await checkAllowed()) {
      await axios
        .put("/api/courses/status/" + props.data.CourseID)
        .then((res) => {
  ...
  const toogleOnline = () => {
    confirm({
      ...
      onOk() {
        axios
          .get("/api/courses/countstages/" + props.data.CourseID)
          .then((res) => {
            if (res.data[0].totalStages >= 5) {
              changeCourseStatus();
            } else {
              message.warning("Add atleast 5 stages to online your course.");
            }
          });
      },
      onCancel() {
